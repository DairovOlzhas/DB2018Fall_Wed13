...
SELECT ALL * FROM employees WHERE lastname = 'Smith' OR lastname = 'Doe';

SELECT ALL * FROM employees WHERE department = 14;
SELECT ALL * FROM employees WHERE department = 37 OR department = 77;

SELECT sum(budget) FROM departments;
SELECT department, count(*) FROM employees GROUP BY department;
SELECT department, count(*) FROM employees GROUP BY department HAVING count(*) > 2;

SELECT name FROM departments ORDER BY budget DESC NULLS LAST LIMIT 1 OFFSET 1;

SELECT * FROM  departments ORDER BY budget DESC NULLS LAST ;
select name, lastname FROM employees where department = 37;

SELECT name, lastname FROM employees WHERE department IN (SELECT code FROM departments ORDER BY budget ASC NULLS LAST LIMIT 1 OFFSET 0);


SELECT name FROM employees WHERE city = 'Almaty' UNION SELECT name, lastname FROM customers WHERE city = 'Almaty';

SELECT * FROM departments WHERE budget > 60000 ORDER BY budget ASC , code DESC;

UPDATE departments SET budget = budget * 0.9 WHERE budget IN (SELECT budget FROM departments ORDER BY budget ASC NULLS LAST LIMIT 1 OFFSET 0);
